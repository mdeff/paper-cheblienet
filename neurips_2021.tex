...
\haguettaz{\subsection{Why using controllable orientation anisotropies?}}

\haguettaz{To justify to work with the full Lie group}


\begin{figure*}[h!] 
\centering
    \centering
    \includegraphics[width=\textwidth]{Images/orientation_cifar10.png}
    \caption{Edges sampling method with $\kappa_\mathcal{E} = 0.6$: 1) a p.m.f. based on the edges' weight is computed 2) $\kappa_\mathcal{E} |\mathcal{E}|$ are sampled according to the p.m.f.}
    \label{fig:sampling}
\end{figure*}

\haguettaz{\subsection{Why using controllable spatial anisotropies?}}
