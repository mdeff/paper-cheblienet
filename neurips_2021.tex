...
\section{Conclusion} \label{sec:scope_limitations_future_research}
...
\item The rate of edges or vertices to sample greatly impact the performance of the model. However, we notice that the model remains stable even at middle-low rates of sampling. At low rates, the information flows throughout the graphs are too disturbed to let the model learn anything. We should also be aware that the effect of the sampling rates also depends on the connectivity of the graph. Indeed, as the number of edges increases, we should theoretically use an lower sampling rate as the flow throughout a highly connected graph is less impacted by random closures of edges or vertices.
\end{itemize}
}
\end{document}
