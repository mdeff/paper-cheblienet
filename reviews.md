...
1. *Clarification.* From the reviews, we note that some parts of our work need to be clarify. We will make effort in our revision to rectify the missing notation and definition. We will also discuss in more depth theoretical results and give a more explicit presentation of our method and its core components. All of you gave excellent clues on where to make improvements in this direction.
2. *Comparison with the state-of-the-art.* We understand that more experiments with the state-of-the-art would make our analysis more rigourous. However, our main motivation when we started working on this subject was to develop a method to turn equivariant an initially invariant spectral graph NN in order to exploit anisotropies in data. We found that an empirical proof of the necessity of using directional information was more important than engineering the neural network architecture to make them better than the current state-of-the-art. This is what we did: we evaluated various neural networks with different degrees of anisotropies and we showed that for some tasks, we could benefit a lot from extending the initial isotropic base space to an isotropic one. 
